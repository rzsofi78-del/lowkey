...
const deobfuscate = async (path, outFile = "out.lua") => {
    ...
    if (!MainFunction) {
        ...
        process.stderr.write("Error: yeah no this aint prometheus.\n:(\n");
        process.exit(1);
    }
    ...
    if (!Env) {
        process.stderr.write("Error something aint right most likely not prometheus.\n");
        process.exit(1);
    }
    ...
    // Final post-processing: remove anti-tamper stubs, dead locals, etc.
    try {
        postprocess(output)
    } catch (err) {
    ...
    // Constant table array folding (local t = {a,b,c}; t[1] → a)
    try {
        optimize(output)
    } catch (err) {
    ...
    try {
        ...
        console.error("failed to fix post-optimize indexes", err)
    }
    ...
    await fs.writeFile(outFile, `-- ts was generated at tuff\n\n` + formattedText);
}
