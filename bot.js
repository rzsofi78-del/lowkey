    ...
    if (message.content.startsWith('.deobf')) {
        ...
        if (message.attachments.size > 0) {
            ...
            if (attachment.name.endsWith('.lua') || attachment.name.endsWith('.txt')) {
                ...
                return message.reply("Please provide a `.lua` or `.txt` file.");
            }
        }
            ...
            try {
                ...
                if (refMsg.attachments.size > 0) {
                    ...
                    if (attachment.name.endsWith('.lua') || attachment.name.endsWith('.txt')) {
                        ...
                        return message.reply("The replied-to message has no `.lua` or `.txt` attachment.");
                    }
                } else if (link) {
                    fileUrl = link;
                } else {
        ...
        // ── 3. Use a provided link
            ...
            return message.reply(
                "**Usage:**\n" +
                "• Attach a `.lua`/`.txt` file and run `.deobf`\n" +
                "• Reply to a message with a `.lua`/`.txt` file and run `.deobf`\n" +
                "• Run `.deobf <raw_url>`"
            );
        }

        // ── Start processing
        const statusMsg = await message.reply("Deobfuscating twinnie...");
        ...
        try {
            ...
            try {
                ...
                await statusMsg.edit('Deobfuscation complete twinnie');
